...
int imm_convert(char *file, char *bin){
    char file1[4], bin1[4];
    int i;
    verify_format(file, file1);
    verify_format(bin, bin1);
    if(!strcmp(file1, "txt")){   // conversao de imm para txt, ou de txt para txt;
        if(!strcmp(bin1, "txt")){    // caso de conversao de txt para txt;
            i = segment_txt_2_txt(file, bin, -1);
        }else if(!strcmp(bin1, "imm")){  // caso de conversao de txt para imm;
    ...
    else if(!strcmp(file1, "imm")){  // conversao de imm para txt, ou de imm para imm;
        if(!strcmp(bin1, "imm")){  // caso de conversao de imm para imm;
            i = segment_bin_2_bin(file, bin, -1);
        }else if(!strcmp(bin, "txt")){ // caso de conversao de imm para txt;
...
int imm_segment(char *file, char *bin, int thr){
    ...
    if(!strcmp(file1, "txt")){  // verificacao de segmentacao de txt para txt, ou de txt para imm;
        if(!strcmp(file2, "txt")){  // caso de segmentacao de txt para txt;
            i = segment_txt_2_txt(file, bin, thr);
        }else if(!strcmp(file2, "imm")){   // caso de segmentacao de txt para imm;
    ...
    }else if(!strcmp(file1, "imm")){  // verificacao de segmentacao de imm para imm, ou de imm para txt;
        if(!strcmp(file2, "imm")){  // caso de segmentacao de imm para imm;
            i = segment_bin_2_bin(file, bin, thr);
        }else if(!strcmp(file2, "txt")){  // caso de segmentacao de imm para txt;

...
TImg *open_txt_file(char file[]){ //funcao de abrir um arquivo de texto;
    ...
    while(p != '\n'){ //este while conta as colunas do arquivo, por isso roda por apenas uma linha;
        p = fgetc(tf);
        if(p == '\t'){ //dependendo da forma da imagem, isto pode nao funcionar, no caso da imagem.txt, a primeira linha é toda composta por 255, mas se fosse uma linha ao centro, onde tem valores menores que 100, poderia contabilizar errado;
            col++;
        }
    }
    ...
    while(p != EOF){ // o while verifica primeiro e depois lê o caractere;
        ...
        if(p == '\n'){ //casos onde foi achado o fim da linha
            ...
            if(val == 3){
                v1 = (int)value[0]; //separando cada casa decimal por variavel, para ficar um pouco mais limpo o codigo;
                v2 = (int)value[1]; 
                v3 = (int)value[2]; // casos onde se leu 3 caracteres de pixel;
            }
            else if(val == 2){
                v2 = (int)value[0];
                v3 = (int)value[1]; // casos onde se leu 2 caracteres de pixel;
            }
            else if(val == 1){
                v3 = (int)value[0]; // casos onde se leu apenas 1 caractere de pixel;
            }
    ...
    img_set_value(img, i, j+1, -2); //setando a ultima posição com -2 para se saber que chegou ao fim do TAD, apesar de ser possivel verificar de outras formas;
    fclose(tf);
    return img;
}
